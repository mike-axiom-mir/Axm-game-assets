...
def run() -> None:
    ...
    assert package["schema"] == "axm.game-assets.hm08-full-body-current.v0.5"
    ...
    boots = package["boot_rubber_structure"]
    assert boots["schema"] == "axm.game-assets.hm08-boot-rubber-structure.v0.1"
    assert boots["toe_component_count"] == 2
    assert boots["truth"]["body_grounded"] is True
    assert boots["truth"]["canonical_body_mutated"] is False
    assert boots["truth"]["source_boot_upper_replaced"] is False
    assert boots["uv_validation"]["status"] == "pass"
    assert boots["topology"]["nonmanifold_edges"] == 0
    for side in ("left", "right"):
        row = boots["side_evidence"][side]
    ...
    assert names[5] == "AXM_Sentinel_BootRubber_v0_1"
    ...
    print("HM08 CURRENT EQUIPPED BODY V0.5 PASS", {
        ...
        "boot_toe_components": boots["toe_component_count"],
