...
def run() -> None:
    ...
    assert first["schema"] == "axm.game-assets.hm08-full-rifle-pose.v0.2"
    assert first["pose_id"] == "full_rig_cross_chest_low_ready_v0.2"
    assert first["base_rig_schema"] == "axm.game-assets.hm08-humanoid-rig.v0.1"
    assert first["joint_count"] == 23
    assert first["weapon"]["scale"] == [1.0, 1.0, 1.0]
    assert first["weapon"]["translation_error_m"] < 1e-6, first["weapon"]

    sockets = first["character_hand_sockets"]
    for side in ("right", "left"):
        assert sockets[side]["name"] == f"{side}_palm_grip"
        assert len(sockets[side]["position"]) == 3
        assert len(sockets[side]["rotation"]) == 4
    ...
    for side in ("right", "left"):
        ...
        # Socket truth is exact. Mesh truth is intentionally separate because
        # these are first-pass full-body skin weights, not final finger wrap.
        assert row["centroid_to_socket_error_m"] < 0.080, row
        pose = first["pose"][side]
        assert 0.17 < pose["upper_bind_length_m"] < 0.25
        assert 0.18 < pose["forearm_bind_length_m"] < 0.30
        assert pose["character_hand_socket"]["name"] == f"{side}_palm_grip"
    ...
    assert truth["contact_only_bones_added"] is False
    assert truth["character_side_hand_sockets_used"] is True
    ...
    print("HM08 FULL RIG RIFLE SOCKET POSE TEST PASS", {
        "primary_error_mm": contact["primary_position_error"] * 1000.0,
        "support_error_mm": contact["support_position_error"] * 1000.0,
        "weapon_translation_error_mm": first["weapon"]["translation_error_m"] * 1000.0,
        "right_hand_mesh_error_mm": first["hand_geometry"]["right"]["centroid_to_socket_error_m"] * 1000.0,
        "left_hand_mesh_error_mm": first["hand_geometry"]["left"]["centroid_to_socket_error_m"] * 1000.0,
        "edge_ratio": edge,
