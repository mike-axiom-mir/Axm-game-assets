...
def run() -> None:
    ...
    assert first["schema"] == "axm.game-assets.hm08-sentinel-rigid-armor.v0.1"
    assert 1.4 <= first["body_bounds_m"]["height"] <= 2.2
    assert first["primary_component_count"] >= 14, first
    assert first["accent_component_count"] >= 8, first
    for name in ("chest_core", "back_core", "left_pauldron", "right_pauldron", "left_forearm_guard", "right_forearm_guard", "left_knee_plate", "right_knee_plate", "left_shin_plate", "right_shin_plate"):
        assert name in first["primary_components"], (name, first["primary_components"])
    for name in ("chest_spine", "left_chest_rail", "right_chest_rail", "back_spine", "left_forearm_rail", "right_forearm_rail", "left_shin_rail", "right_shin_rail"):
    ...
    with TemporaryDirectory() as a, TemporaryDirectory() as b:
        ...
        for material_name in ("primary", "accent"):
            ...
            assert ah == bh
        assert ma["primary"]["spec"]["paint_rgb"] == [48, 58, 64]
        assert ma["accent"]["spec"]["paint_rgb"] == [23, 28, 31]
