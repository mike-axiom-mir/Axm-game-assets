...
def run() -> None:
    ...
    skin = first["shared_rig"]["finger_skin_evidence"]
    ...
    assert reference["arm_joint_rotations_preserved"] is True
    assert reference["weapon_scale"] == [1.0,1.0,1.0]
    assert float(reference["primary_position_error_m"]) < 1e-8
    assert float(reference["support_position_error_m"]) < 1e-6
    assert grip["open_53_joint_finger_skin_vs_v0_4_max_mesh_delta_m"] > 0.0
    ...
    assert first["truth"]["v0_4_mesh_identity_claim"] is False
    ...
    print("HM08 FINGER GRIP V0.5 TEST PASS", {
        ...
        "finger_skin_vs_v4_delta_mm": grip["open_53_joint_finger_skin_vs_v0_4_max_mesh_delta_m"] * 1000.0,
